...
import { getEpigramById, editEpigram } from '@/api/epigram';
...
export default function EditEpigram() {
  ...
  const params = useParams(); // /epigrams/[id]/edit 라우트일 경우
  ...
  // 기존 데이터 가져오기
  const { data: epigram, isLoading } = useQuery({
    queryKey: ['epigram', id],
  ...
  useEffect(() => {
    if (epigram) {
      reset({
        ...
        referenceUrl: '', // 서버에서 내려줄 경우 채우기
        referenceTitle: '',
        tags: epigram.tags?.map((t) => t.name) ?? [],
      });
      setTags(epigram.tags?.map((t) => t.name) ?? []);

      if (['custom', 'unknown', 'user'].includes(epigram.author)) {
        setSelectedAuthor(epigram.author as 'custom' | 'unknown' | 'user');
      } else {
  ...
  if (!epigram) return <div>데이터가 없습니다</div>;
  ...
  return (
    <div className='bg-white min-h-screen'>
      <div className='flex justify-center items-start flex-col m-auto w-[312px] md:w-[384px] lg:w-[640px] pt-[24px] md:pt-[32px] lg:pt-[56px] pb-[30px]'>
        <h3 className='text-lg md:text-xl lg:text-2xl font-semibold'>에피그램 수정하기</h3>
        <form
